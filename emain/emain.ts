...
console.log(
    sprintf(
        "genieterm-app starting, data_dir=%s, config_dir=%s electronpath=%s gopath=%s arch=%s/%s electron=%s",
        waveDataDir,
        waveConfigDir,
        getElectronAppBasePath(),
        getElectronAppUnpackedBasePath(),
...
if (isDev) {
    console.log("genieterm-app WAVETERM_DEV set");
}
...
electronApp.on("before-quit", (e) => {
    ...
    if (
        ...
        const choice = electron.dialog.showMessageBoxSync(null, {
            ...
            buttons: ["Cancel", "Quit"],
            title: "Confirm Quit",
            message: "Are you sure you want to quit GenieTerm?",
            defaultId: 0,
            cancelId: 0,
        });
        if (choice === 0) {
