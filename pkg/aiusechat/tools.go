...
func MakeBlockShortDesc(block *waveobj.Block) string {
	...
	case "cpuplot":
		...
		return "cpu graph"
	case "tips":
		return "GenieTerm quick tips widget"
	case "help":
		return "GenieTerm documentation widget"
	case "launcher":
	...
	case "waveconfig":
		if file, hasFile := block.Meta["file"].(string); hasFile && file != "" {
			return fmt.Sprintf("GenieTerm config editor for %q", file)
		}
		return "GenieTerm config editor"
	default:
